...
import { generateStudyPlan, getMyStudyPlans, StudyPlan } from '@/lib/api/study-planner';
  ...
  const handleGenerate = async () => {
    ...
    try {
      await generateMutation.mutateAsync({
        ...
        weakTopics: weakTopicsInput.split(',').map((t) => t.trim()).filter(Boolean),
      });
    } catch {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Get a personalized 7-day study schedule based on your subjects and upcoming exams
        </p>
      </div>
      ...
      <Card>
        <CardContent className="space-y-4 pt-6">
          ...
          <Button onClick={handleGenerate} disabled={!profile || generateMutation.isPending}>
                ...
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {displayPlan.priorityTopics.map((topic) => (
                      <span key={topic} className="rounded-full bg-muted px-3 py-1 text-xs">
                        {topic}
                      </span>
                    ))}
                  </div>
                      ...
                      {tasks.map((task, i) => (
                        <div key={i} className="flex items-center justify-between rounded-md border p-3 text-sm">
                          <div>
                            ...
                            <p className="text-muted-foreground">{task.durationMinutes} minutes</p>
                          </div>
