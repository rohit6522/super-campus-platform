...
async function seed() {
  ...
  const faculty = await userModel.create({
    ...
    email: 'faculty@seed.test',
    password: hashedPassword,
    role: UserRole.FACULTY,
  });

  const placementOfficer = await userModel.create({
    name: 'Placement Officer',
    email: 'placement@seed.test',
    password: hashedPassword,
  ...
  const studentUser2 = await userModel.create({
    name: 'Priya Verma',
    email: 'student2@seed.test',
    password: hashedPassword,
    role: UserRole.STUDENT,
  });

  console.log('Creating subjects...');
  const subject1 = await subjectModel.create({
  ...
  const subject2 = await subjectModel.create({
